...
references/
  ...
  pian-risk-checklist.md     ← 10-pattern lint checklist applied to lyrics
  style-templates.md         ← ready-made Styles strings per sub-genre
  examples.md                ← end-to-end worked outputs
```
...
- **Warn, don't auto-fix.** The skill flags เพี้ยน risks and proposes rewrites but lets the user choose. This is stated in `references/pian-risk-checklist.md` and `references/tonal-melodic-rules.md`. New rules must follow the same pattern — never add logic that silently rewrites user lyrics.
